...
if(isset($_GET['loginEmail'])){
    $x=$_GET['loginEmail'];
    if ($x == 0)
        echo "<p id='logEmail' hidden  >fail</p>";
}
if(isset($_GET['loginPass'])){
    $x=$_GET['loginPass'];
    if ($x == 0)
        echo "<p id='logPass' hidden  >fail</p>";
}
if(isset($_GET['login'])) {
    $x=$_GET['login'];
    if ($x == 0)
...
if(isset($_GET['signupFn'])){
    $x=$_GET['signupFn'];
    if ($x == 0)
        echo "<p id='signupFn' hidden  >fail</p>";
}
if(isset($_GET['signupLn'])){
    $x=$_GET['signupLn'];
    if ($x == 0)
        echo "<p id='signupLn' hidden  >fail</p>";
    
}
if(isset($_GET['signupEmail'])){
    $x=$_GET['signupEmail'];
    if ($x == 0)
        echo "<p id='signupEmail' hidden  >fail</p>";
    
}
if(isset($_GET['signupPass'])){
    $x=$_GET['signupPass'];
    if ($x == 0)
        echo "<p id='signupPass' hidden  >fail</p>";
            ...
            <div class="row rowx no-pad-bot card-content no-pad-top ">
                <div class="input-field col s12 no-pad-bot center ">
                  <!--  <img src="image/logo.png" alt="" class=" responsive-img valign profile-image-login">-->
                    <p class="center-align text-darken-4 white-text  " style="margin: 0px;">LOST AND FOUND WEBSITE</p>
                </div>
            </div>
...
<div class="row"rowx >
<form method="post" action="guestpage.php">
    <button class="btn waves-effect waves-light blue  col s12">
